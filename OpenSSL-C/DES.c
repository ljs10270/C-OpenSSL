...
int main() {
	...
	printf("DES 알고리즘으로 암/복호화를 시작 합니다.");

	const EVP_CIPHER *algorism = EVP_des_cbc(); //암호화에 사용될 알고리즘과 모드 생성
	...
	unsigned char *cecret_key = (unsigned char*)malloc(EVP_CIPHER_CTX_key_length(ctx_enc));//DES 알고리즘에 사용될 비밀키 공간 동적 할당
	...
	FILE *cfp = fopen("C:/Users/LJS/Desktop/Computer Science/Information Security/DES_암호문.txt", "w");
	...
	cfp = fopen("C:/Users/LJS/Desktop/Computer Science/Information Security/DES_암호문.txt", "r");
	pfp = fopen("C:/Users/LJS/Desktop/Computer Science/Information Security/DES_복호문.txt", "w");
...
int Cryptology(const unsigned char *cecret_key, const unsigned char *iv, FILE *ifp, FILE *ofp, int isEncrypt) {
	...
	if (!(EVP_CipherInit_ex(ctx_enc, EVP_des_cbc(), NULL, cecret_key, iv, isEncrypt))) {
